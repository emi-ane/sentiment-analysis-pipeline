...
def train_epoch(model, data_loader, loss_fn, optimizer, scheduler, device):
    ...
    return correct_predictions.double() / len(data_loader.dataset), np.mean(losses), all_predictions, all_targets
...
def train():
    ...
    if not all(col in df.columns for col in required_columns):
        logging.error(f"Dataset missing required columns. Expected: {required_columns}, Got: {list(df.columns)}")
        return
