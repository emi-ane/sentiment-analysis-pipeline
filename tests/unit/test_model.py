...
def test_training_step(model, sample_dataset):
    ...
    batch = {
        'input_ids': torch.stack([sample_dataset[0]['input_ids'], sample_dataset[1]['input_ids']]).to(device),
        'attention_mask': torch.stack([sample_dataset[0]['attention_mask'], sample_dataset[1]['attention_mask']]).to(device),
        'targets': torch.tensor([sample_dataset[0]['targets'], sample_dataset[1]['targets']]).to(device)
    }
